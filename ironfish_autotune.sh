#!/bin/bash
# ============================================================
#  IronFish Auto-Tune Script
#  Coin: IronFish (IRON)
#  Algorithm: Blake3
#  Miner: lolMiner
# ============================================================

LOG_FILE="ironfish_autotune_results.csv"
BEST_FILE="ironfish_best.txt"

echo "GPU Model,Core Clock (MHz),Power Limit (W),Hashrate (MH/s),Power Draw (W),Efficiency (MH/W)" > "$LOG_FILE"
echo "IronFish Auto-Tune started at $(date)"
echo "------------------------------------------------------------"
...
benchmark_gpu() {
    ...
    HASHRATE=$(timeout 45s lolMiner --benchmark IRON 2>/dev/null | grep "H/s" | tail -n 1 | awk '{print $3}')
...
echo "" > "$BEST_FILE"
echo "🏆 Best Settings (IronFish)" >> "$BEST_FILE"
echo "========================" >> "$BEST_FILE"
